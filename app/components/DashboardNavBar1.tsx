...
import { ArrowDown, LucideIcon, SearchIcon } from "lucide-react";
...
const DEFAULT_SELECT_OPTIONS = [
  { value: "profile", label: "View Profile", icon: <IoPerson /> },
  { value: "settings", label: "Settings", icon: <IoSettings /> },
...
export default function DashboardNavBar1({
  ...
  const mergedAvatarArea: AvatarAreaProps = {
    ...{
      ...
      onSelectChange: (value) => console.log("Selected:", value),
    },
  ...
  const handleSelectChange = (value: string) => {
    ...
    const selectedOption = mergedAvatarArea.selectOptions?.find(
    ...
    >
      ...
      {!leftSection ? (
        <div className="flex items-center text-xl max-md:hidden">
          ...
          <h2 className="font-normal px-1">{mergedAvatarArea.username}</h2>
        </div>
      ) : (
        <>{leftSection}</>
      )}
      ...
      <div className="flex items-center space-x-4">
        ...
        <Button
          onClick={
            mergedButtonData.onClickedBtn && mergedButtonData.onClickedBtn
          }
          className={`h-10 shadow-none ${mergedButtonData.className}`}
        >
          <mergedButtonData.icon />
          <span className="max-sm:hidden">{mergedButtonData.text}</span>
        </Button>
        ...
        {mergedAvatarArea.showAvatar && (
          <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-fit p-0" align="start">
              <Command>
                <CommandList>
                  <CommandGroup heading={mergedAvatarArea.username}>
                    {mergedAvatarArea.selectOptions?.map((option) => (
                      <CommandItem
                        key={option.value}
                        onSelect={() => {
